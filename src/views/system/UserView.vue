<template>
    <div>
        <div style="display: flex; justify-content: space-between; margin-top: 10px">
            <div>
                <el-input prefix-icon="el-icon-search" style="width: 350px; margin-right: 10px" />
                <el-button type="primary" icon="el-icon-search">搜索</el-button>
            </div>
            <div>
                <el-button type="primary" icon="el-icon-view" @click="toShowView">查看</el-button>
                <el-button type="success" icon="el-icon-plus" @click="toAddView">添加</el-button>
                <el-button type="warning" icon="el-icon-edit" @click="toEditView">编辑</el-button>
                <el-button type="danger" icon="el-icon-delete" @click="deleteUser">删除</el-button>
            </div>
        </div>
        <!--  用户列表  -->
        <div style="margin-top: 10px">
            <el-table :data="userList" stripe border v-loading="loading" style="width: 100%"
                highlight-current-row @current-change="selectCurrentRow">
                <el-table-column type="selection" />
                <el-table-column prop="username" label="用户名" />
                <el-table-column prop="nickname" label="昵称" />
                <el-table-column prop="email" label="邮箱" />
                <el-table-column prop="userface" label="用户头像" />
                <el-table-column prop="description" label="描述" />
                <el-table-column prop="lastLoginTime" label="登录时间" />
                <el-table-column label="状态">
                    <template slot-scope="scope">
                        <el-switch v-model="scope.row.enabled" />
                    </template>
                </el-table-column>
            </el-table>
            <el-pagination background
                @current-change="pageChange"
                layout="prev, pager, next, total"
                :total="total" />
        </div>
        <!--  查看  -->
        <div>
            <el-dialog :title="title" :visible.sync="showModalVisible" width="50%">
                <el-form :model="user" label-position="right" label-width="80px">
                    <el-row>
                        <el-col :span="12">
                            <el-form-item prop="username" label="用户名">
                                <el-input type="text" v-model="user.username" style="width: 80%" disabled />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item prop="nickname" label="昵称">
                                <el-input type="text" v-model="user.nickname" style="width: 80%" disabled />
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row>
                        <el-col :span="12">
                            <el-form-item prop="email" label="电子邮箱">
                                <el-input type="text" v-model="user.email" style="width: 80%" disabled />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="角色">
                                <el-select v-model="checkedIds" multiple style="width: 80%" disabled>
                                    <el-option v-for="item in roleList"
                                        :key="item.id"
                                        :label="item.description"
                                        :value="item.id" />
                                </el-select>
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row>
                        <el-form-item prop="userface" label="用户头像">
                            <el-input v-model="user.userface" style="width: 90%" disabled />
                        </el-form-item>
                    </el-row>
                    <el-row>
                        <el-form-item prop="description" label="个人说明">
                            <el-input type="textarea" v-model="user.description" style="width: 90%" disabled />
                        </el-form-item>
                    </el-row>
                </el-form>
            </el-dialog>
        </div>
        <!--  添加/修改  -->
        <div>
            <el-dialog :title="title" :visible.sync="editModalVisible" width="50%">
                <el-form :model="user" ref="user" :rules="rules" label-position="right" label-width="80px">
                    <el-row>
                        <el-col :span="12">
                            <el-form-item prop="username" label="用户名">
                                <el-input type="text" v-model="user.username" placeholder="请输入用户名"
                                    prefix-icon="el-icon-edit" style="width: 80%" />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item prop="nickname" label="昵称">
                                <el-input type="text" v-model="user.nickname" placeholder="请输入昵称"
                                    prefix-icon="el-icon-edit" style="width: 80%" />
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row>
                        <el-col :span="12">
                            <el-form-item prop="email" label="电子邮箱">
                                <el-input type="text" v-model="user.email" placeholder="请输入电子邮箱"
                                    prefix-icon="el-icon-message" style="width: 80%" />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="角色">
                                <el-select v-model="checkedIds" multiple placeholder="请选择" style="width: 80%">
                                    <el-option v-for="item in roleList"
                                        :key="item.id"
                                        :label="item.description"
                                        :value="item.id" />
                                </el-select>
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row>
                        <el-form-item prop="userface" label="用户头像">
                            <el-input v-model="user.userface" placeholder="请上传用户头像" style="width: 90%" />
                        </el-form-item>
                    </el-row>
                    <el-row>
                        <el-form-item prop="description" label="个人说明">
                            <el-input type="textarea" v-model="user.description" placeholder="请输入个人说明"
                                style="width: 90%" />
                        </el-form-item>
                    </el-row>
                </el-form>
                <span slot="footer" class="dialog-footer">
                    <el-button @click="editModalVisible = false">取消</el-button>
                    <el-button type="primary" @click="saveUser">确定</el-button>
                </span>
            </el-dialog>
        </div>
    </div>
</template>

<script>
export default {
    name: 'UserView',
    data() {
        return {
            loading: false,
            page: 0,
            size: 10,
            total: 0,
            userList: [],
            roleList: [],
            title: '',
            showModalVisible: false,
            editModalVisible: false,
            user: {
                username: '',
                //password: '',
                nickname: '',
                email: '',
                userface: '',
                description: '',
                enabled: false
            },
            checkedIds: [],
            rules: {
                username: [{required: true, message: '请输入用户名', trigger: 'blur'},
                    {min: 4, max: 16, message: '长度必须是4-16个字符'}
                ],
                password: null,
                nickname: [{required: true, message: '请输入昵称', trigger: 'blur'}],
                email: [{required: true, message: '请输入电子邮箱', trigger: 'blur'},
                    {type: 'email', message: '邮箱格式不正确', trigger: 'blur'}
                ],
                userface: null,
                description: null
            }
        }
    },
    mounted() {
        this.initUserList()
        this.initRoleList()
    },
    methods: {
        selectCurrentRow(val) {
            this.currentRow = val
        },
        pageChange(page) {
            this.page = page - 1
            this.initUserList();
        },
        initUserList() {
            this.loading = true
            let url = '/system/user/listUsersByPage?page=' + this.page + '&size=' + this.size
            this.getRequest(url)
            .then(resp => {
                this.loading = false
                if(resp && resp.code == 200) {
                    this.userList = resp.data.content
                    this.total = resp.data.total
                }
            })
        },
        initRoleList() {
            this.getRequest('/system/role/listRoles')
            .then(resp => {
                if(resp && resp.code == 200) {
                    this.roleList = resp.data
                }
            })
        },
        emptyUser() {
            this.user = {
                username: '',
                //password: '',
                nickname: '',
                email: '',
                userface: '',
                description: '',
                enabled: false
            }
            this.checkedIds = []
        },
        saveUser() {
            //更新用户
            if(this.user.id){
                this.$refs.user.validate(valid => {
                    if(valid) {
                        let form = this.user
                        delete form.lastLoginTime
                        form.checkedIds = this.checkedIds
                        this.putRequest('/system/user/updateUser', form)
                        .then(resp => {
                            if(resp && resp.code == 200) {
                                this.$message({
                                    message: '修改成功',
                                    type: 'success'
                                })
                                this.editModalVisible = false
                                this.initUserList()
                            }
                        })
                    }
                })
            //保存用户
            } else {
                this.$refs.user.validate(valid => {
                    if(valid) {
                        let form = this.user
                        delete form.lastLoginTime
                        form.checkedIds = this.checkedIds
                        this.postRequest('/system/user/saveUser', form)
                        .then(resp => {
                            if(resp && resp.code == 200) {
                                this.$message({
                                    message: '保存成功',
                                    type: 'success'
                                })
                                this.editModalVisible = false
                                this.initUserList()
                            }
                        })
                    }
                })
            }
        },
        //查看
        toShowView() {
            if (this.currentRow == null) {
                this.$message({
                    message: '请选择一项',
                    type: 'warning'
                })
                return
            }
            this.title = '查看用户信息'
            this.user = this.currentRow
            this.getRequest('/system/role/listRidsByUid?uid=' + this.currentRow.id)
            .then(resp => {
                if(resp && resp.code == 200) {
                    this.checkedIds = resp.data
                }
            })
            this.showModalVisible = true
        },
        //添加
        toAddView() {
            this.title = '添加用户信息'
            this.emptyUser()
            this.editModalVisible = true
        },
        //修改
        toEditView() {
            if (this.currentRow == null) {
                this.$message({
                    message: '请选择一项',
                    type: 'warning'
                })
                return
            }
            this.title = '修改用户信息'
            this.user = this.currentRow
            this.getRequest('/system/role/listRidsByUid?uid=' + this.currentRow.id)
            .then(resp => {
                if(resp && resp.code == 200) {
                    this.checkedIds = resp.data
                }
            })
            this.editModalVisible = true
        },
        //删除
        deleteUser() {
            if (this.currentRow == null) {
                this.$message({
                    message: '请选择一项',
                    type: 'warning'
                })
                return
            }
            this.$confirm('此操作将永久删除此项, 是否继续?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                this.deleteRequest('/system/user/deleteUser?id=' + this.currentRow.id)
                .then((resp) => {
                    if (resp) {
                        this.$message({
                            message: '删除成功',
                            type: 'success'
                        })
                        this.initUserList()
                    }
                })
            }).catch(() => {
                this.$message({
                    message: '已取消',
                    type: 'info'
                })
            })
        }
    }
}
</script>